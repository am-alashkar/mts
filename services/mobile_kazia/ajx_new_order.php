...
function wfa_pre_price_calc($prices) {
    $price = $prices['wfa_pre_price'];
    $adder = $prices['wfa_pre_type'];
    $add_am = $prices['wfa_pre_adder'];
    $amount = data::$get->amount;
    $total = 0;
    if ($amount > 0)
    {
...
function wfa_pos_builder($bill) {
    $all['info'] = $bill->to_array();
    $all['info']['type_html'] = 'جملة Wafatel لاحق الدفع';
...
function wfa_pre_builder($bill) {
    $all['info'] = $bill->to_array();
    $all['info']['type_html'] = 'جملة WAFATEL مسبق الدفع';
    $all['info']['company'] = 'WAFATEL';
    $all['info']['price'] = $bill->price;
    $all['info']['infos'] = 'الكمية : '.$bill->amount.'<br>الرقم : '.$bill->phone .'<br>الكود : '.data::$get->code;
    $field['name'] = 'الشركة';
...
function wfa_cash_builder($bill) {
    $all['info'] = $bill->to_array();
    $all['info']['type_html'] = 'جملة WAFATEL كاش';
    $all['info']['company'] = 'WAFATEL';
    $all['info']['price'] = $bill->price;
    $all['info']['infos'] = 'المبلغ : '.$bill->amount.'<br>الرقم : '.$bill->phone.'<br>الكود : '.data::$get->code;
    $field['name'] = 'الشركة';
