...
if ($info) {
    ...
    if ($found_info[$fk][0]) {
        $r = $found_info[$fk][0];
        if ($r['note'] == 'مبلغ ثابت') {
            $final_array['mtn_pre_adder'] = $r['adder'];
        }
        $final_array['mtn_pre_price'] = $r['price'];
    }
    $fk = 'Syriatel مسبق الدفع';
    if ($found_info[$fk][0]) {
        $r = $found_info[$fk][0];
        if ($r['note'] == 'مبلغ ثابت') {
            $final_array['syt_pre_adder'] = $r['adder'];
        }
        $final_array['syt_pre_price'] = $r['price'];
    }
    $fk = 'Wafatel مسبق الدفع';
    if ($found_info[$fk][0]) {
        $r = $found_info[$fk][0];
        if ($r['note'] == 'مبلغ ثابت') {
            $final_array['wfa_pre_adder'] = $r['adder'];
        }
        $final_array['wfa_pre_price'] = $r['price'];
    }
...
//$c_s = db::$db->adv_select('*','custom_prices',$sql);
    if (is_array($final_array)) {
        ...
        $in['script'] = 'mobile_kazia';
        $in['user_id'] = $user_id;
        db::$db->insert('custom_prices',$in);
        $final_ans .= ' جملة كازيات<br>';
    }
}
