...
if ($mem->deleted || $mem->disabled)
{
    ...
    $tmp = explode("\n",(string) $prs['mtn_cities_in']);
    foreach ($tmp as $item) {
        $prices['mtn_cities'] .= '<option value="'.$item.'">'.$item.'</option>';
        $prices['mtn_cits'][] = $item;
    }
    $tmp = explode("\n",(string) $prs['wfa_cities_in']);
    foreach ($tmp as $item) {
        $prices['wfa_cities'] .= '<option value="'.$item.'">'.$item.'</option>';
        $prices['wfa_cits'][] = $item;
    }
    ...
    if (+$prs['wfa_pos_price_0'] >0 && +$prs['wfa_pos_price_'.$mem->price_group] >0) {
        ...
        if ($prices['wfa_pos_type'] == 'true') {
            $prices['wfa_pos_price'] =  +$prs['syt_pos_price_0'];
    ...
    if (+$prs['wfa_pre_price_0'] >0 && +$prs['wfa_pre_price_'.$mem->price_group] >0) {
        $prices['wfa_pre_type'] = $prs['wfa_pre_type'];
        if ($prices['wfa_pre_type'] == 'true') {
            $prices['wfa_pre_price'] =  +$prs['wfa_pre_price_0'];
            $prices['wfa_pre_adder'] = +$prs['wfa_pre_price_' . $mem->price_group];
            if (isset($custom_prices['wfa_pre_adder'])) {
                if ($custom_prices['wfa_pre_price']<1) {
                    unset($prices['wfa_pre_price']);
                    unset($prices['wfa_pre_adder']);
                    //unset($prices['syt_cash_type']);
                } else {
                    $prices['wfa_pre_adder'] = +$custom_prices['wfa_pre_adder'];
                    $prices['wfa_pre_price'] = +$custom_prices['wfa_pre_price'];

                }
            }
        } else
        {
            $prices['wfa_pre_price'] = max(+$prs['wfa_pre_price_' . $mem->price_group], +$prs['wfa_pre_price_0']);
            $prices['wfa_pre_adder'] = 0;
            if (isset($custom_prices['wfa_pre_price']) ) {
                if ($custom_prices['wfa_pre_price']<1) {
                    unset($prices['wfa_pre_price']);
                    unset($prices['wfa_pre_adder']);
                    //unset($prices['mtn_pre_type']);
                } else {
                    $prices['wfa_pre_price'] = max(+$custom_prices['wfa_pre_price'], +$prs['wfa_pre_price_0']);
                }
            }
        }
    }
