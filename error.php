<?php
if ($_GET['job'] == 'api') {
    print $error['details'].' '.$error['code'];
    die();
}
error_reporting(0);
...
if ($_GET['error']) {
    $error['code'] = '403';
    $error['details'] = 'YOU ARE Not allowed to define the error.';
    $error['files'] = '🐱 🐱‍🚀';
}
if (!$error) {
    $error['code'] = '403';
    $error['details'] = 'Not Allowed to see error page without an error defined.';
    $error['files'] = 'Confused Confuser 😵';
}
if (!$error['files']) {
    $error['files'] = 'UFO 🛸';
}
...
try {
    // @TODO : CHECK THIS if auto loader is not yet loaded
    if (phrases::$error_header[$error['code']]) {
        header(phrases::$error_header[$error['code']],true);
...
?>
    <html><head>
        ...
        <style>
            ...
            body div#error div#box:after {
                ...
                animation: dance 2s infinite steps(20);
            }
            body div#error div#box:before {
            ...
            body div#error h3 {
                ...
                animation: blink 2.3s infinite steps(100);
            }
            body div#error h3:before,
            body div#error h3:after {
                content: 'ERROR <?php print $error['code'] ?>';
                position: absolute;
                top: -1px;
                left: 0;
    ...
    <div id="error">
        <div id="box"></div>
        <h3>ERROR <?php print $error['code'] ?></h3>
        <hr>
        <p>Things are a little <span>unstable</span> here</p>
        <p>
            <?php

            print '<b>'.$error['details'].'</b><br /> Error page generated by '.$error['files'];
